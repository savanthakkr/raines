...
export default function EmployeeProfileTab() {
    ...
    const [showPassedModal, setShowPassedModal] = React.useState(false);
    const [showFailedModal, setShowFailedModal] = React.useState(false);
    const [selectedResult, setSelectedResult] = useState(""); // Controls the dropdown value
    ...
    useEffect(() => {
        if (router.pathname === '/business/employees/manage-employees') {
            setActiveTab('Details');
        } else if (router.pathname === '/business/employees/employees-course') {
            setActiveTab('Course');
        } else if (router.pathname === '/business/employees/course-report') {
            setActiveTab('Report');
        }
    }, [router.pathname]);
    ...
    const handleTabChange = (tab) => {
        setActiveTab(tab);
        if (tab === 'Details') {
            router.push('/business/employees/personal-details');
        } else if (tab === 'Course') {
            router.push('/business/employees/employees-course');
        } else if (tab === 'Report') {
            router.push('/business/employees/course-report');
        }
    };
                            ...
                            {courses.map((user, idx) => (
                                <tr key={idx} className="border-t">
                                    ...
                                    <td className="p-3 text-center">
                                        <select id="result" name="result" className="result_dropdown" value={selectedResult[idx] || ""} onChange={(e) => {
                                            const value = e.target.value;
                                            setTempResult({ idx, value });
                                            if (value === "passed") {
                ...
                <div className="login-form">
                    <div className="modal-overlay">
                        <div className="modal-content">
                            <div className="forget-pwd-form text-center">
                                <img src="/images/img/assign-result-modal-icon.svg" />
                                <h4 className="mt-1 mb-2">Assign Result</h4>
                                <h6 className="mb-0">Are you sure you want to mark the student as Passed?</h6>
                                <div className="d-flex flex-row justify-content-between mt-5">
                                    <button className="default-btn back-btn" onClick={() => { setShowPassedModal(false); setTempResult({}); }}>Cancel<span></span></button>
                                    <button className="default-btn" onClick={() => {
                                        setSelectedResult((prev) => ({
                                            ...prev,
                                            [tempResult.idx]: tempResult.value
                                        }));
                                        setShowPassedModal(false);
                                        setTempResult({});
                                    }}>Confirm Passed<span></span></button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="modal-overlay">
                        <div className="modal-content">
                            <div className="forget-pwd-form text-center">
                                <img src="/images/img/assign-result-modal-icon.svg" />
                                <h4 className="mt-1 mb-2">Assign Result</h4>
                                <h6 className="mb-0">Are you sure you want to mark the student as Failed?</h6>
                                <div className="d-flex flex-row justify-content-between mt-5">
                                    <button
